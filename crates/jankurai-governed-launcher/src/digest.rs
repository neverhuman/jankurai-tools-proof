use super::{filesystem::read_bounded, BoundaryError, Result, MAX_BINARY_BYTES};
use std::fs::File;
...
pub(super) fn require_digest(bytes: &[u8], expected: &str, message: &'static str) -> Result<()> {
...
pub(super) fn require_file_digest(file: &File, expected: &str) -> Result<()> {
    let mut copy = file
        .try_clone()
